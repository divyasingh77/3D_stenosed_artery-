...
void coordinate_mapping_function(libMesh::Point& X,
...
static double lambda_s = 1e6;
static double mu_s = 1e5;
...
void PK1_dev_stress_function(TensorValue<double>& PP,
                            ...
                            void*)
{
    const double mu_s = ModelData::mu_s;
    const int dim = NDIM;
    ...
    // 2. Calculate Inverse Transpose of Deformation Gradient (F^-T)
    // Note: For 3D, libMesh has built-in inverse. 
    // If FF is nearly singular, this might need a check.
    TensorValue<double> FF_inv_trans = FF.inverse().transpose();

    // 3. Neo-Hookean Stress (Standard for IBFE)
    // P = mu * (F - F^-T)
